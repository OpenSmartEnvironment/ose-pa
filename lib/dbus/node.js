 ...
 */
  ...
  switch (value) {
  case 'down':
    value = e.sval.value + 0.01;
    if (value < 0) value = 0;

    break;
  case 'up':
    value = Math.round(e.sval.value + e.sval.max * 0.01);
    if (value > e.sval.max) {
      value = e.sval.max;
    }
  ...
  value = Math.round(value * e.sval.max);
...
function openBus(entry, address) {  // {{{2
  ...
  function onSinks(err, val) {  // {{{3
    if (err) {
      O.log.error(err);
      return;
    }

    entry.sinkName = val[1][0][0];
  ...
  function onMute(err, val) {  // {{{3
    if (err) {
      O.log.error(entry, err);
    } else {
      muteUpdated.call(entry, val[1][0]);
    }
  }

  function onVolume(err, val) {  // {{{3
    if (err) {
      O.log.error(entry, err);
    } else {
      volumeUpdated.call(entry, val[1][0]);
    }
  }

  function onVolumeSteps(err, val) {  // {{{3
    if (err) {
      O.log.error(entry, err);
    } else {
      entry.setState({steps: val[1][0]});
    }
  }

  function onBaseVolume(err, val) {  // {{{3
    if (err) {
      O.log.error(entry, err);
    } else {
      entry.setState({max: val[1][0]});
    }
  }

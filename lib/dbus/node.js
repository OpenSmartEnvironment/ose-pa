  ...
  if (this.child && (this.child.pid === pid)) {
    for (var key in data) {
      ...
      case 'streamMute':
        ...
        break;
      default:
        O.log.unhandled('Unhandled Pulse data', data);
      }
    }
  }
};
 ...
 */
  ...
  if (
    ...
    O.log.warning('Invalid value, should be: (0 <= value <= 1)', value); // TODO: Answer to socket.
  }
};
...
function connect(that) {  // {{{2
  ...
  if (that.child) {  // Clean old child. {{{3
    O.log.notice('Closing old child.');
    console.trace();

//    that.child.removeAllListeners();
//    that.child.on('error', O.dummyFn);
    that.child.kill();
  ...
  that.connectHandle = setTimeout(function() {  // Timeout connection by that.connectTimeout {{{3
    ...
    O.log.notice('Connecting to new child.');
    ...
    */
    ...
    O.log.debug('Spawning new child');
  }, that.connectTimeout);
...
function send(that, data) {  // {{{2
  if (that.child) {
    ...
    O.log.unhandled('Pulse send error', that.identify);
  }
}

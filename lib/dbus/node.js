...
function readAddress(entry) {  // {{{2
  ...
  Fs.stat('/run/pulse/dbus-socket', function(err, stat) {
    ...
    bus.invoke({
      ...
      if (err) {
        M.log.error(err, 'Unable to connect to PulseAudio D-Bus socket', 3);
        setTimeout(readAddress.bind(null, entry, 10000));
        return;
      }
...
function openBus(entry, address) {  // {{{2
  entry.bus = Dbus.createClient({busAddress: 'unix:path=/run/user/1000/pulse/dbus-socket', direct: true});
  var svc = entry.bus.getService('org.PulseAudio.Core1');

  svc.getInterface('/org/pulseaudio/core1', 'org.freedesktop.DBus.Properties', function(err, iface) {
